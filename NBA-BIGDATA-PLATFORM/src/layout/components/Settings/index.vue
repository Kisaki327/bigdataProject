  ...
  <el-drawer size="280px" :visible="showSettings" :with-header="false" :append-to-body="true" :before-close="closeSetting" :lock-scroll="false">
    <div class="drawer-container">
      <div>
        ...
        <h3 class="drawer-title">AI 图表解读</h3>

        <div class="drawer-item">
          <span>选择图表</span>
          <el-select v-model="selectedChartId" placeholder="请选择图表" style="width: 100%; margin-top: 8px;">
            <el-option
              v-for="(chart, id) in chartList"
              :key="id"
              :label="(chart.option && chart.option.title && chart.option.title.text) || id"
              :value="id"
            />
          </el-select>
        </div>
        <div class="drawer-item">
          <el-button type="primary" icon="el-icon-magic-stick" @click="analyzeChart" :loading="loading" size="small">生成解读</el-button>
        </div>
        <el-card class="drawer-item" v-if="aiResult">
...
export default {
  ...
  computed: {
    ...
    chartList() {
    return this.$store.state.chart.charts;
  }
  },
  methods: {
    themeChange(val) {
